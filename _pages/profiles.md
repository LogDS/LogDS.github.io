...
{% if member.github %} <a href="{{ member.github }}" target="_blank"><i class="fab fa-github fa-2x"></i></a> {% endif %}
...
<div class="col-sm-4 col-xs-12">
  <h4>{{ member.name }}</h4>
  {{ member.duration }} <br> Role: <i>{{ member.role }}</i>. "{{ member.dissertation }}" {{ member.info }}
  <ul style="overflow: hidden">
  </ul>
</div>
